...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-outomidea-pipeline` | Outomidea V4 파이프라인의 에이전트 핸드오프 및 일관성 검증 | `c:/hehe/outomidea/1.music_video/**/*.md` |
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
...
1. **연관 스킬 파일 업데이트** — 새 스킬 생성 후 반드시 아래 3개 파일을 업데이트합니다:
...
1. 업데이트된 각 스킬에서 탐지 명령어 하나를 드라이런하여 문법 유효성 검증
2. **등록된 검증 스킬** 테이블과 **실행 대상 스킬** 테이블이 동기화되어 있는지 확인
