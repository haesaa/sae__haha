...
| 기본 | 이름, 나이, 소속/직업/직위 |
| 외형 | **필수 6항목 (키 제외 반드시 성격과 연관 지어 묘사할 것):** ①눈 형태+색, ②헤어스타일, ③성격을 반영한 구체적 체형(예: 넓은 어깨, 두터운 몸, 얇은 허리, 슬랜더 등), ④키(**※남성 캐릭터는 무조건 181cm ~ 199cm 사이로 하드코딩 필수**), ⑤주로 입는 의상/패션 스타일, ⑥자주 짓는 특유의 표정/인상 |
...
- max_tokens: 8000
- 캐릭터 수 하드코딩 금지
- 남성 캐릭터의 키는 무조건 181cm ~ 199cm 사이로 하드코딩 필수
- 한국어 작성 (CAUTION만 영문)
- 기법 지시문 줄당 ≤100자
