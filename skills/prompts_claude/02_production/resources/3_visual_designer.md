...
#### 🎨 공통 화풍 (이미지 첨부 시 must 작성 — 미첨부 시 공란)

> 에이전트 창에서 이미지가 첨부된 경우, 해당 이미지의 **그림체·색감·채색 방식**을 분석하여 아래 항목을 반드시 채운다.
> 이미지가 없으면 이 섹션은 빈 블록으로 유지한다.

```text
[이미지 첨부 시 여기에 그림체·화풍·색감·채색 방식을 자세하게니지저니 태그 형식으로 작성]
예: soft watercolor style, warm pastel palette, loose ink linework, impressionistic brush strokes, muted earth tones
```

> ⚠️ 이 섹션에 작성된 태그는 모든 캐릭터의 기본 틀 **⑦ 공통 후미 태그 앞에 삽입**한다.
...
#### 🎨 NanoBanana2 공통 화풍 (이미지 첨부 시 must 작성 — 미첨부 시 공란)

> 에이전트 창에서 이미지가 첨부된 경우, 해당 이미지의 **그림체·색감·채색 방식**을 분석하여 아래 항목을 반드시 채운다.
> 이미지가 없으면 이 섹션은 빈 블록으로 유지한다.
> 작성된 태그는 모든 캐릭터 Positive prompt 맨 앞에 삽입한다.

```text
[이미지 첨부 시 여기에 그림체·화풍·색감·채색 방식을 NanoBanana2 태그 형식으로 작성]
예: (anime style, soft shading, warm color palette, detailed cel shading:1.2)
```

---

#### 🔒 공통 Negative (전 캐릭터 고정)

```text
(worst quality, low quality:1.4), deformed, bad anatomy, bad proportions, missing fingers, extra limbs, fused fingers, text, watermark, signature, logo
```

---

#### NanoBanana2 기본 틀 생성 규칙

각 캐릭터의 NanoBanana2 프롬프트는 Positive / Negative 두 줄로 구성한다.

- **Positive 첫머리**: `(masterpiece, best quality, highres:1.2),` 고정 — 공통 화풍 태그가 있으면 그 뒤에 삽입
- **캐릭터 묘사 순서**: 성별 → 체형(신장·체중감·체격) → 헤어 → 눈 → 표정·인상 → 복장·소품 → 배경·조명
- **강조치**: 핵심 특징에 `(tag:1.2~1.4)` 적용 가능 (남용 금지)
- **Negative**: 공통 Negative + 맞지 않는 체형/헤어색/배경 등 캐릭터별 추가

---

#### NanoBanana2 캐릭터별 출력 형식
...
- **Positive:**
```
(masterpiece, best quality, highres:1.2), [공통화풍태그 — 있는 경우], [성별·체형], [헤어], [눈·표정], [복장·소품], [배경·조명]
```

- **Negative:**
```
(worst quality, low quality:1.4), deformed, bad anatomy, bad proportions, missing fingers, [캐릭터별 추가 금지 요소]
```
````
